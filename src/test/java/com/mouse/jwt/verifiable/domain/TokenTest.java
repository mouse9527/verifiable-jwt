...
class TokenTest {
    ...
    void shouldBeAbleToCreateWithJWTToken() throws IOException {
        ...
        assertThat(token.getPayload(TestPayload.class).getId()).isEqualTo("mock-token-id");
        assertThat(token.getPayload(TestPayload.class).getType()).isEqualTo("user");
        assertThat(token.getPayload(TestPayload.class).getIat()).isNotEmpty();
        assertThat(token.getPayload(TestPayload.class).getExp()).isNotEmpty();
